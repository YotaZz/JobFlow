...
import { Eye, ShieldCheck, ArrowRight, Loader2, UserPlus, LogIn, Lock } from 'lucide-react';
import { supabase } from '../supabaseClient';

interface WelcomeModalProps {
  isOpen: boolean;
  onClose?: () => void; // 用于非强制模式下的关闭
  onLoginSuccess?: () => void; // 登录成功回调
  onSelectView: (email: string) => void;
  initialMode?: 'selection' | 'auth'; // 支持指定初始模式
}
...
export const WelcomeModal: React.FC<WelcomeModalProps> = ({ 
  ...
  const [mode, setMode] = useState<'selection' | 'view-input' | 'auth'>(initialMode);
    ...
    switch (mode) {
      case 'view-input':
        return {
          ...
          title: '查看他人进度',
          subtitle: '请输入目标用户的邮箱地址'
        };
      case 'auth':
        return {
          bg: 'bg-gradient-to-br from-blue-600 to-indigo-700',
          title: isSignUp ? '注册新账号' : '管理员登录',
          subtitle: isSignUp ? '创建账号以开始追踪您的求职进度' : '登录以管理您的投递记录'
        };
      default: // selection
        return {
          bg: 'bg-gradient-to-br from-blue-600 to-indigo-700',
          ...
          {mode === 'selection' && (
            <div className="space-y-4 animate-in slide-in-from-left duration-300">
              <button 
                ...
                className="w-full group relative flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-blue-500 hover:bg-blue-50 transition-all text-left"
              >
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-slate-800">我要管理</h3>
                  <p className="text-xs text-slate-500 mt-1">登录/注册账号，创建和管理我的投递记录</p>
                </div>
                <ArrowRight size={18} className="text-slate-300 group-hover:text-blue-600" />
              </button>

              <button 
                onClick={() => setMode('view-input')}
                className="w-full group relative flex items-center gap-4 p-4 rounded-xl border border-slate-200 hover:border-emerald-500 hover:bg-emerald-50 transition-all text-left"
              >
                ...
                <div className="flex-1">
                  <h3 className="font-bold text-slate-800">仅查看</h3>
                  <p className="text-xs text-slate-500 mt-1">输入邮箱，查看该用户的公开投递进度</p>
                </div>
            ...
            <form onSubmit={handleViewSubmit} className="space-y-4 animate-in slide-in-from-right duration-300">
              <div>
                <input 
                  ...
                  className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-center text-lg"
                  value={viewEmail}
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button 
                  type="submit" 
                  className="flex-1 py-2.5 bg-emerald-600 text-white font-bold rounded-lg hover:bg-emerald-700 shadow-lg shadow-emerald-200"
                >
                  查看记录
                </button>
              </div>
                ...
                <form onSubmit={handleAuthSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-3 pt-2">
                    ...
                    <button 
                      type="submit" 
                      disabled={loading}
                      className="flex-1 py-2.5 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 disabled:opacity-50 flex justify-center items-center gap-2 shadow-lg shadow-blue-200"
                    >
