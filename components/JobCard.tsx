...
export const JobCard: React.FC<JobCardProps> = ({ job, onUpdateStep, onEdit, onDelete }) => {
      ...
      <div className="p-3">
        ...
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className={`
                flex-shrink-0 px-1.5 py-0.5 text-[10px] font-bold rounded border
                ${getBadgeStyle(job.jobType)}
              `}>
            ...
            <div className="flex items-center gap-3 text-xs text-slate-500">
               {job.salary && (
                <div className="flex items-center gap-1 text-emerald-600 bg-emerald-50/50 px-1.5 rounded">
                  <Wallet size={10} />
                  <span className="font-medium truncate max-w-[100px]" title={`正职待遇: ${job.salary}`}>{job.salary}</span>
                </div>
              )}
        ...
        <div className="bg-slate-50 rounded-md border border-slate-100/50 relative">
            ...
            <Stepper 
              ...
              interactive={true}
              onStepClick={(idx) => onUpdateStep(job.id, idx)}
            />
        </div>
