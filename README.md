...
无论是实习、校招还是社招，你都可以自定义求职节点，通过**灵活的标签系统**进行分组管理，记录薪资待遇、面试备注，并实时追踪从“已投递”到“OC”的全过程及**面试效率分析**。
...
| **支持按标签（如招聘季）分组查看与搜索** | **手动日期校准与拖拽式节点管理** |
...
👉 [**点击进入 JobFlow - 求职投递管理**](https://jobflow.yotazzz.xyz/)
...
### 🎯 核心管理
* **🏷️ 灵活标签系统 (New)**：支持自定义标签（如“2026秋招”、“外企”、“保底”），并支持按标签**分组查看**和统计，替代传统的固定招聘季分类。
* **📊 面试效率分析 (New)**：自动计算每个岗位的**面试平均耗时**（天/轮），直观评估流程推进效率。
* **📅 手动时间校准 (New)**：支持在节点配置中手动修改每个步骤的发生日期，确保记录精准无误（默认为操作时的当前时间）。
* **👥 多用户支持**：完整的注册/登录体系，数据云端同步，多端可查。

### ⚡ 流程追踪
* **👀 仅查看模式**：支持通过输入邮箱查看特定用户的公开投递进度（只读），方便分享与进度汇报。
* **📊 可视化进度条**：通过动态进度条展示面试阶段，支持“进入”、“等待”、“挂掉”三种状态的直观切换。
* **🤖 自动化流程管理**：
  * **Ghosting Detection**：若“初筛”阶段超过 **10天** 无回应，自动标记为“无回应/已挂”。
  * **自动时间轴**：自动记录操作时间，同时支持手动修正。

### ⚙️ 个性化配置
* **📝 多维度信息**：记录公司、岗位、薪资（支持复杂公式）、备注信息。
* **🔧 自定义流程**：支持全局配置默认面试流程（如：笔试 -> 一面 -> 二面 -> HR面），也可针对单个岗位微调。
...
本项目依赖 Supabase 的 PostgreSQL 数据库。若进行私有化部署，请在 Supabase 的 **SQL Editor** 中执行以下命令。

<details>
<summary>点击展开 SQL 配置代码 (Updated)</summary>
...
create table public.jobs (
  ...
  tags text[] default '{}', -- 标签分组 (替代原 season 字段)
  salary text null,
  notes text null,
